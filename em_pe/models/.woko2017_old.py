...
class woko2017(model_base):
    ...
    def evaluate(self, tvec_days, band):
        ...
        for ii in xrange(len(ints)):
            ...
            if ii == 0:
                f1 = interp1d(t, np.log10(data[:,idx1]), fill_value='extrapolate')
                f2 = interp1d(t, np.log10(data[:,idx2]), fill_value='extrapolate')
            else:
                f1 = interp1d(t,data[:,idx1], fill_value='extrapolate')
                f2 = interp1d(t,data[:,idx2], fill_value='extrapolate')
