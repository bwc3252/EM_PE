...
class kilonova_2c(model_base):
    def __init__(self):
        name = "kilonova_32"
        param_names = ["mej_polar",
                       "mej_eq",
                       "vej_polar",
                       "vej_eq",
                       "kappa_polar",
    ...
    def set_params(self, params, t_bounds):
        ...
        ### completely arbitrary cutoff temperature in K
        Tc = 4000.0

        ### for each component, compute photosphere radius and temperature
        for mej, vej, kappa in zip(
                [params["mej_polar"], params["mej_eq"]],
                [params["vej_polar"], params["vej_eq"]],
                [params["kappa_polar"], params["kappa_eq"]]):
            a = self.fa([mej, vej])[0]
            b = self.fb([mej, vej])[0]
            d = self.fd([mej, vej])[0]
            vej *= c
    ...
    def evaluate(self, tvec_days, band):
        ...
        F_nu = np.zeros(self.tdays.size)
        for _R_photo, _T_photo, profile_factor in zip(self.R_photo, self.T_photo, [1.0 - np.cos(self.theta), np.cos(self.theta)]):
            B_nu = (2.0 * h * nu**3 / c**2) / np.expm1(h * nu / (kb * _T_photo))
            F_nu += profile_factor * B_nu * np.pi * _R_photo**2 / D**2
