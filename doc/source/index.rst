...
bands are used::
    ...
    python em_pe/parser/parse_json.py --t0 1187008882.43 --f Data/GW170817.json \
                            --b H --b J --out temp/ --maxpts 100 --tmax 8 \
                            --time_format gps

    n=75 # fix number of iterations to something reasonable
...
lightcurve plot (again as a bash script)::
    ...
    python -m em_pe/plot_utils/plot_lc --tmin 0.5 --tmax 8 --out temp/lc.png \
        --posterior_samples $dir/H/posterior_samples.txt \
        --posterior_samples $dir/J/posterior_samples.txt \
        --b H \
        --b J \
        --lc_file temp/H.txt \
        --lc_file temp/J.txt \
...
The Python equivalent to the previous example::
    ...
    samples = ['temp/H/posterior_samples.txt', 'temp/J/posterior_samples.txt']
    out = temp/corner.png
